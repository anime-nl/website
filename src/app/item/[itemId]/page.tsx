...
export default async function ItemPage({ params }: { params: Promise<{ itemId: string }> }) {
	...
	const description = await remark()
		.use(remarkHtml)
		.process(item.description ?? 'Er is geen beschrijving voor dit product beschikbaar');
	...
	return (
		<div className="dark mx-16 pt-16 min-h-screen font-[family-name:var(--font-geist-sans)]">
			<main className="flex flex-col gap-16 items-center h-full w-full">
				<ItemInfo description={description.toString()} item={item} specifications={specifications.toString()} />
			</main>
		</div>
	);
}
